...
这是一个**单文件插件项目**，所有逻辑在 `main.py` 中（约 550 行），无独立依赖管理文件，依赖由 AstrBot 框架提供。

核心组成：

- `detect_image_type()` — 通过 magic number 识别图片格式（JPEG/PNG/GIF/WebP/BMP/TIFF）
- `ImageStore` — 数据持久化层，SQLite 存元数据 + 文件系统存图片，asyncio.Lock 保护并发写入
- `LocalImageBedPlugin(Star)` — 插件主类，继承 AstrBot 的 `Star` 基类
...
```
上传 → Token 鉴权 → 读取 payload → detect_image_type → SHA256 去重检查
     → 写入文件系统 + INSERT SQLite → 返回 {id, url}
...
- 配置通过 `self.config.get(key, default)` 读取，对应 `_conf_schema.json` 中定义的 6 个配置项
...
`listen_host`、`listen_port`、`public_base_url`、`upload_token`、`max_upload_mb`、`enable_deduplicate`
